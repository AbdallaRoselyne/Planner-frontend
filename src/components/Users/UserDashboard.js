...
const CalendarPage = () => {
  ...
  useEffect(() => {
    fetchApprovedTasks();
  }, [showAllTasks]); // Re-fetch tasks when toggle state changes

  const fetchApprovedTasks = async () => {
    try {
      const email = localStorage.getItem("email"); // Retrieve email from localStorage
      const endpoint = showAllTasks
        ? "http://localhost:5000/api/tasks?status=Approved" // Fetch all approved tasks
        : `http://localhost:5000/api/tasks/user/${email}?status=Approved`; // Fetch tasks for the logged-in user
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="mb-6">
        ...
        >
          {showAllTasks ? "Show My Calendar" : "Show Everyone's Calendar"}
        </button>
      </div>
